...
var presionaPieza = function presionaPieza(fila, columna) {
    ...
    if (matrizDesorganiza[fila][columna].ocupado) {
        ...
        for (var i in direcciones) {
            ...
            //console.log("Posicion de busqueda: "+posBusca);
            ...
            if (enPosicion) {
                ...
                //console.log("Entregadas: "+ fila + columna);

                if (!matrizDesorganiza[posBusca.fila][posBusca.columna].ocupado) //pregunta si esta vacio
                    {
                        //console.log("Sumadas: "+ posBusca.fila + posBusca.columna);

                        var estiloimg = _utils2.default.accesoDOM(fila + "_" + columna).style;
                        var estiloespacio = _utils2.default.accesoDOM(posBusca.fila + "_" + posBusca.columna).style;

                        //estilos para la nueva posición
                        _utils2.default.accesoDOM(posBusca.fila + "_" + posBusca.columna).style.width = estiloimg.width;
                        _utils2.default.accesoDOM(posBusca.fila + "_" + posBusca.columna).style.height = estiloimg.height;
                        _utils2.default.accesoDOM(posBusca.fila + "_" + posBusca.columna).style.display = estiloimg.display;
                        _utils2.default.accesoDOM(posBusca.fila + "_" + posBusca.columna).style.border = estiloimg.border;
                        _utils2.default.accesoDOM(posBusca.fila + "_" + posBusca.columna).style.backgroundPosition = estiloimg.backgroundPosition;
                        _utils2.default.accesoDOM(posBusca.fila + "_" + posBusca.columna).style.cursor = estiloimg.cursor;
                        _utils2.default.accesoDOM(posBusca.fila + "_" + posBusca.columna).style.boxShadow = estiloimg.boxShadow;

                        _utils2.default.accesoDOM(posBusca.fila + "_" + posBusca.columna).className = "fondo";
                        _utils2.default.accesoDOM(posBusca.fila + "_" + posBusca.columna).innerHTML = _utils2.default.accesoDOM(fila + "_" + columna).innerHTML;
                        matrizDesorganiza[posBusca.fila][posBusca.columna].ocupado = true;
                        matrizDesorganiza[fila][columna].ocupado = false;

                        //estilos para la posición que debe quedar vacia
                        _utils2.default.accesoDOM(fila + "_" + columna).className = "";
